...
def getEuclidean(point1, point2):
    ...
    for i in range(dimension):
        try:
            term1=point1[i]
            term2=point2[i]
            dist += (float(term1) - float(term2)) ** 2
        except:
            print("exception")
...
def k_means_clustering(data,k):
    ...
    vectors = []
    for i in index:
        vectors.append(data[i])   ##centroids
    print(len(data))
    labels = np.zeros((len(data),1))
    for i in range(len(data)):
        labels[i][0]=-1       #centroids or not
    while True:
        centers=np.zeros((k,len(data[0])))
        pre=np.zeros((k,len(data[0])))
        num=np.zeros(k)
        num.fill(0)
        cluster_heart=np.zeros((k,len(data[0])))
        cluster_heart.fill(0)
        ...
        for labelIndex, item in enumerate(data):
            ...
            for i,coordinate in enumerate(item):
                cluster_heart[classIndex][i]+=coordinate
            
            labels[labelIndex] = classIndex
        for i in range(k):
            for j in range(len(data[0])):
                centers[i][j]=cluster_heart[i][j]/num[i]
        comparision=pre==centers
        check=comparision.all()
        if(check):
            break
        pre=centers
...
def k_means_segmentation(img, k):
    ...
    if len(img.shape)==2:
        h,w=img.shape
        imge=np.reshape(img,(h*w,1))
    else:
        h,w,d=img.shape
        imge=np.reshape(img,(h*w,d))
       
    labels,centers=k_means_clustering(imge,k)
